...
class ChromaConnection(BaseConnection):
    ...
    def renew_key(self, key: str) -> int:
        ...
        resp = requests.get(urljoin(self.base_url, "/auth/renew"), params=renew_payload)
    ...
    def revoke_key(self, key: str) -> int:
        ...
        resp = requests.get(
            urljoin(self.base_url, "/auth/revoke"), params=revoke_payload
        )
        if resp.status_code == 200:
    ...
    def retrieve_documents(self, key, query, n_docs=1) -> list[str] | int:
        ...
        docs = requests.get(
            urljoin(self.base_url, "retrieve"), headers=headers, params=payload
        )
